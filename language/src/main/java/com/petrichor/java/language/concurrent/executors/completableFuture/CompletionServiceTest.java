package com.petrichor.java.language.concurrent.executors.completableFuture;
...
//        case2();
     ...
     * 3. 如果到了任务列表末端, 那么就重新回到任务列表开始, 然后继续从第一步开始执行
     * <p>
     * 这样就可以及时处理已完成任务的结果了
     */
     ...
     * a. ExecutorCompletionService内部维护列一个队列, 用于管理已完成的任务
     * b. 内部还维护列一个Executor, 可以执行任务
     * <p>
     * 2. ExecutorCompletionService内部维护了一个BlockingQueue, 只有完成的任务才被加入到队列中
     * <p>
     * 3. 任务一完成就加入到内置管理队列中, 如果队列中的数据为空时, 调用take()就会阻塞 (等待任务完成)
     * a. 关于完成任务是如何加入到完成队列中的, 请参考ExecutorCompletionService的内部类QueueingFuture的done()方法
     * <p>
     * 4. ExecutorCompletionService的take/poll方法是对BlockingQueue对应的方法的封装, 关于BlockingQueue的take/poll方法:
     * a. take()方法, 如果队列中有数据, 就返回数据, 否则就一直阻塞;
     * b. poll()方法: 如果有值就返回, 否则返回null
     * c. poll(long timeout, TimeUnit unit)方法: 如果有值就返回, 否则等待指定的时间; 如果时间到了如果有值, 就返回值, 否则返回null
     * <p>
     * 解决了已完成任务得不到及时处理的问题
     */
